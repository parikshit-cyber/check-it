/**
 * CHECK-IT — Scan. Detect. Protect.
 * Frontend Application Logic (Refactored)
 * Handles user info display, runtime timer, theme toggle, live news map,
 * phishing analysis, IP intelligence, and email phishing analysis.
 */
...
(function () {
  ...
  const statRuntime = $('#stat-runtime');
  ...
  function updateRuntime() {
    ...
    statRuntime.textContent = [hrs, mins, secs].map(v => String(v).padStart(2, '0')).join(':');
  }
  ...
  async function fetchUserInfo() {
    try {
      ...
      statIP.textContent = data.ip || 'Unknown';
      statIP.title = `${data.city}, ${data.country} (${data.lat}, ${data.lng})`;
      statISP.textContent = data.isp || 'Unknown';
      statISP.title = `${data.region}, ${data.country} — ${data.timezone}`;
    } catch (err) {
      console.warn('Failed to fetch user info:', err);
      statIP.textContent = 'Unavailable';
      statISP.textContent = 'Unavailable';
    }
  }
  fetchUserInfo();
  ...
  function initNewsMap() {
    const container = document.getElementById('news-map-container');
    if (!container) return;

    newsMap = L.map(container, {
      center: [20, 0],
      zoom: 2,
      minZoom: 2,
      maxZoom: 10,
  ...
  async function fetchNews() {
    try {
      ...
      // Clear old markers
      newsMarkers.forEach(m => newsMap.removeLayer(m));
      newsMarkers = [];
      ...
      data.news.forEach(item => {
        ...
        const icon = L.divIcon({
          ...
          html: `<div class="news-pulse-marker" style="--marker-color:${color}">
            <div class="news-marker-dot" style="background:${color}"></div>
            <div class="news-marker-ring" style="border-color:${color}"></div>
          </div>`,
          iconSize: [24, 24],
          iconAnchor: [12, 12],
        });

        const marker = L.marker([item.lat, item.lng], { icon }).addTo(newsMap);
        marker.bindPopup(`
          <div class="news-popup">
            <div class="news-popup-category" style="color:${color}">${item.category.toUpperCase()}</div>
            <div class="news-popup-title">${item.title}</div>
            <div class="news-popup-summary">${item.summary}</div>
  ...
  function renderNewsFeed(newsItems) {
    ...
    newsItems.forEach((item, i) => {
      ...
      const timeStr = [ts.getHours(), ts.getMinutes()].map(v => String(v).padStart(2, '0')).join(':');
      const el = document.createElement('div');
      el.className = 'news-feed-item';
      el.style.animationDelay = `${i * 0.08}s`;
      el.innerHTML = `
        <span class="news-category-dot" style="background:${color}"></span>
        <div class="news-feed-content">
          <span class="news-feed-title">${item.title}</span>
          <span class="news-feed-meta">
            <span class="news-feed-loc">📍 ${item.city}</span>
            <span class="news-feed-source">${item.source}</span>
            <span class="news-feed-time">${timeStr}</span>
          </span>
        </div>
        <span class="news-category-badge" style="color:${color};border-color:${color}">${item.category}</span>
      `;
      el.addEventListener('click', () => {
        if (newsMap) {
          newsMap.setView([item.lat, item.lng], 6);
          const marker = newsMarkers.find(m => {
            const pos = m.getLatLng();
            return Math.abs(pos.lat - item.lat) < 0.01 && Math.abs(pos.lng - item.lng) < 0.01;
          });
